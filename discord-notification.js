...
const discordNotification = new DiscordNotification.DiscordNotification('Web-RobotFramework-Example', process.env.DISCORD_WEBHOOK);
...
function test(state) {
	if (state == 'start') {
		discordNotification
			...
			.addField({name: 'Build Number', value: `web.v.1.${process.env.BUILD}` }) 
